...
public class Menus {
	...
	String addMenuBar() {
		...
		Menu fakeMenu = getMenu("");
		fakeMenu.setEnabled(false);
		Menu newMenu = getMenu(">New", true);
		addPlugInItem(file, "Open...", "ij.plugin.Commands(\"open\")", KeyEvent.VK_O, false);
		addPlugInItem(fakeMenu, "Open Next", "ij.plugin.NextImageOpener", KeyEvent.VK_O, true);
		Menu openSamples = getMenu(">Open Samples", true);
		openSamples.addSeparator();
		addPlugInItem(openSamples, "Cache Sample Images ", "ij.plugin.URLOpener(\"cache\")", 0, false);
		addOpenRecentSubMenu(fakeMenu);
		Menu importMenu = getMenu(">Import", true);
		fakeMenu.addSeparator();
		addPlugInItem(fakeMenu, "Close", "ij.plugin.Commands(\"close\")", KeyEvent.VK_W, false);
		addPlugInItem(fakeMenu, "Close All", "ij.plugin.Commands(\"close-all\")", KeyEvent.VK_W, true);
		addPlugInItem(file, "Save", "ij.plugin.Commands(\"save\")", KeyEvent.VK_S, false);
		saveAsMenu = getMenu(">Save As", true);
		addPlugInItem(fakeMenu, "Revert", "ij.plugin.Commands(\"revert\")", KeyEvent.VK_R,  false);
		fakeMenu.addSeparator();
		addPlugInItem(fakeMenu, "Page Setup...", "ij.plugin.filter.Printer(\"setup\")", 0, false);
		addPlugInItem(fakeMenu, "Print...", "ij.plugin.filter.Printer(\"print\")", KeyEvent.VK_P, false);
		
		//Menu edit = getMenu("Edit");
		addPlugInItem(fakeMenu, "Undo", "ij.plugin.Commands(\"undo\")", KeyEvent.VK_Z, false);
		fakeMenu.addSeparator();
		addPlugInItem(fakeMenu, "Cut", "ij.plugin.Clipboard(\"cut\")", KeyEvent.VK_X, false);
		addPlugInItem(fakeMenu, "Copy", "ij.plugin.Clipboard(\"copy\")", KeyEvent.VK_C, false);
		addPlugInItem(fakeMenu, "Copy to System", "ij.plugin.Clipboard(\"scopy\")", 0, false);
		addPlugInItem(fakeMenu, "Paste", "ij.plugin.Clipboard(\"paste\")", KeyEvent.VK_V, false);
		addPlugInItem(fakeMenu, "Paste Control...", "ij.plugin.frame.PasteController", 0, false);
		fakeMenu.addSeparator();
		addPlugInItem(fakeMenu, "Clear", "ij.plugin.filter.Filler(\"clear\")", 0, false);
		addPlugInItem(fakeMenu, "Clear Outside", "ij.plugin.filter.Filler(\"outside\")", 0, false);
		addPlugInItem(fakeMenu, "Fill", "ij.plugin.filter.Filler(\"fill\")", KeyEvent.VK_F, false);
		addPlugInItem(fakeMenu, "Draw", "ij.plugin.filter.Filler(\"draw\")", KeyEvent.VK_D, false);
		addPlugInItem(fakeMenu, "Invert", "ij.plugin.filter.Filters(\"invert\")", KeyEvent.VK_I, true);
		fakeMenu.addSeparator();
		getMenu(">Selection", true);
		Menu optionsMenu = getMenu(">Options", true);
		
		//Menu image = getMenu("Image");
		Menu imageType = getMenu(">Type");
			...
			fakeMenu.add(imageType);
			
		fakeMenu.addSeparator();
		getMenu(">Adjust", true);
		addPlugInItem(fakeMenu, "Show Info...", "ij.plugin.filter.Info", KeyEvent.VK_I, false);
		addPlugInItem(fakeMenu, "Properties...", "ij.plugin.filter.ImageProperties", KeyEvent.VK_P, true);
		getMenu(">Color", true);
		getMenu(">Stacks", true);
		getMenu(">Stacks>Animation_", true);
		getMenu(">Stacks>Tools_", true);
		Menu hyperstacksMenu = getMenu(">Hyperstacks", true);
		fakeMenu.addSeparator();
		addPlugInItem(fakeMenu, "Crop", "ij.plugin.Resizer(\"crop\")", KeyEvent.VK_X, true);
		addPlugInItem(fakeMenu, "Duplicate...", "ij.plugin.Duplicator", KeyEvent.VK_D, true);
		addPlugInItem(fakeMenu, "Rename...", "ij.plugin.SimpleCommands(\"rename\")", 0, false);
		addPlugInItem(fakeMenu, "Scale...", "ij.plugin.Scaler", KeyEvent.VK_E, false);
		getMenu(">Transform", true);
		getMenu(">Zoom", true);
		getMenu(">Overlay", true);
		fakeMenu.addSeparator();
		getMenu(">Lookup Tables", true);
		
		//Process menu place
		addPlugInItem(fakeMenu, "Smooth", "ij.plugin.filter.Filters(\"smooth\")", KeyEvent.VK_S, true);
		addPlugInItem(fakeMenu, "Sharpen", "ij.plugin.filter.Filters(\"sharpen\")", 0, false);
		addPlugInItem(process, "Find Edges", "ij.plugin.filter.Filters(\"edge\")", 0, false);
		addPlugInItem(fakeMenu, "Find Maxima...", "ij.plugin.filter.MaximumFinder", 0, false);
		addPlugInItem(fakeMenu, "Enhance Contrast...", "ij.plugin.ContrastEnhancer", 0, false);
		getMenu(">Noise", true);
		getMenu(">Shadows", true);
		getMenu(">Binary", true);
		getMenu(">Math", true);
		getMenu(">FFT", true);
		Menu filtersMenu = getMenu(">Filters", true);
		fakeMenu.addSeparator();
		getMenu(">Batch", true);
		addPlugInItem(fakeMenu, "Image Calculator...", "ij.plugin.ImageCalculator", 0, false);
		addPlugInItem(fakeMenu, "Subtract Background...", "ij.plugin.filter.BackgroundSubtracter", 0, false);
		addItem(fakeMenu, "Repeat Command", KeyEvent.VK_R, true);
		
		//Menu analyzeMenu = getMenu("Analyze");
		addPlugInItem(fakeMenu, "Measure", "ij.plugin.filter.Analyzer", KeyEvent.VK_M, false);
		addPlugInItem(fakeMenu, "Analyze Particles...", "ij.plugin.filter.ParticleAnalyzer", 0, false);
		addPlugInItem(fakeMenu, "Summarize", "ij.plugin.filter.Analyzer(\"sum\")", 0, false);
		addPlugInItem(fakeMenu, "Distribution...", "ij.plugin.Distribution", 0, false);
		addPlugInItem(fakeMenu, "Label", "ij.plugin.filter.Filler(\"label\")", 0, false);
		addPlugInItem(fakeMenu, "Clear Results", "ij.plugin.filter.Analyzer(\"clear\")", 0, false);
		addPlugInItem(fakeMenu, "Set Measurements...", "ij.plugin.filter.Analyzer(\"set\")", 0, false);
		fakeMenu.addSeparator();
		addPlugInItem(fakeMenu, "Set Scale...", "ij.plugin.filter.ScaleDialog", 0, false);
		addPlugInItem(fakeMenu, "Calibrate...", "ij.plugin.filter.Calibrator", 0, false);
		addPlugInItem(fakeMenu, "Histogram", "ij.plugin.Histogram", KeyEvent.VK_H, false);
		addPlugInItem(fakeMenu, "Plot Profile", "ij.plugin.Profiler(\"plot\")", KeyEvent.VK_K, false);
		addPlugInItem(fakeMenu, "Surface Plot...", "ij.plugin.SurfacePlotter", 0, false);
		getMenu(">Gels", true);
		Menu toolsMenu = getMenu(">Tools", true);

		// the plugins will be added later, with a separator
		//addPluginsMenu();

		//Menu window = getMenu("Window");
		addPlugInItem(fakeMenu, "Show All", "ij.plugin.WindowOrganizer(\"show\")", KeyEvent.VK_CLOSE_BRACKET, false);
		String key = IJ.isWindows()?"enter":"return";
		addPlugInItem(fakeMenu, "Main Window ["+key+"]", "ij.plugin.WindowOrganizer(\"imagej\")", 0, false);
		addPlugInItem(fakeMenu, "Put Behind [tab]", "ij.plugin.Commands(\"tab\")", 0, false);
		addPlugInItem(fakeMenu, "Cascade", "ij.plugin.WindowOrganizer(\"cascade\")", 0, false);
		addPlugInItem(fakeMenu, "Tile", "ij.plugin.WindowOrganizer(\"tile\")", 0, false);
		fakeMenu.addSeparator();

		//Menu help = getMenu("Help");
		addPlugInItem(fakeMenu, "ImageJ Website...", "ij.plugin.BrowserLauncher", 0, false);
		addPlugInItem(fakeMenu, "ImageJ News...", "ij.plugin.BrowserLauncher(\""+IJ.URL+"/notes.html\")", 0, false);
		addPlugInItem(fakeMenu, "Documentation...", "ij.plugin.BrowserLauncher(\""+IJ.URL+"/docs\")", 0, false);
		addPlugInItem(fakeMenu, "Installation...", "ij.plugin.SimpleCommands(\"install\")", 0, false);
		addPlugInItem(fakeMenu, "Mailing List...", "ij.plugin.BrowserLauncher(\"https://list.nih.gov/archives/imagej.html\")", 0, false);
		fakeMenu.addSeparator();
		addPlugInItem(fakeMenu, "Dev. Resources...", "ij.plugin.BrowserLauncher(\""+IJ.URL+"/developer/index.html\")", 0, false);
		addPlugInItem(fakeMenu, "Plugins...", "ij.plugin.BrowserLauncher(\""+IJ.URL+"/plugins\")", 0, false);
		addPlugInItem(fakeMenu, "Macros...", "ij.plugin.BrowserLauncher(\""+IJ.URL+"/macros/\")", 0, false);
		addPlugInItem(fakeMenu, "Macro Functions...", "ij.plugin.BrowserLauncher(\""+IJ.URL+"/developer/macro/functions.html\")", 0, false);
		Menu examplesMenu = getExamplesMenu(ij);
		fakeMenu.add(examplesMenu);
		fakeMenu.addSeparator();
		addPlugInItem(fakeMenu, "Update ImageJ...", "ij.plugin.ImageJ_Updater", 0, false);
		addPlugInItem(fakeMenu, "Refresh Menus", "ij.plugin.ImageJ_Updater(\"menus\")", 0, false);
		fakeMenu.addSeparator();
		Menu aboutMenu = getMenu(">About Plugins", true);
		addPlugInItem(fakeMenu, "About ImageJ...", "ij.plugin.AboutBox", 0, false);
		fakeMenu.setEnabled(false);
